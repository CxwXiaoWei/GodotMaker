...
| 升级类型 | 处理方式 |
|--------------|--------------|
| **Patch**（如 0.3.0 → 0.3.1） | 自动执行——只是 bug 修复，无需人工确认 |
| **Minor**（如 0.3.0 → 0.4.0） | 展示新版本的变更日志，要求确认后执行迁移脚本 |
| **Major**（如 0.x → 1.x） | 需要加 `--force`——有破坏性变更，需要全新初始化 |
| **版本相同** | 始终执行——适合在本地修改框架后重新部署 |
| **降级** | 默认阻止，需要 `--force` 才能强制执行 |
