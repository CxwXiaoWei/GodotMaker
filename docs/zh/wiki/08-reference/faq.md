...
- 评估期间截取的截图：`e2e/screenshots/`
- 每个场景的视觉参考图：`e2e/screenshots/scene_<name>/`
...
`/gm-verify` 完成后，结果在 `.godotmaker/verify_result.json` 中。每个 Worker 的报告也记录了其测试是否通过。如果 Godot 无头构建失败或单元测试失败，Verifier 子代理会报告这一情况，问题会反馈给 Worker 进行修复。
